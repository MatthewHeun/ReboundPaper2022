...
\begin{equation}
  \rate{M} = p_s \rate{q}_s + R_\alpha \rate{C}_{cap} + \rate{C}_{\omd} + \rate{C}_g  + \rate{N}
\end{equation}
%
...
\begin{equation} \label{eq:eqspsC_exact}
  \eqspsChat = - \frac{\frac{1}{1 - \rho} \left( 1 - \fCs \right) \left[ \frac{1 - \fCs}{\fCs} \frac{\amacro{p}_s \rorig{q}_s}{\rorig{C}_g} \right] ^ {\rho / (1 - \rho)} }{\fCs + \left( 1 - \fCs \right) \left[ \frac{1 - \fCs}{\fCs} \frac{\amacro{p}_s \rorig{q}_s}{\rorig{C}_g} \right] ^ {\rho / (1 - \rho)} } \; .
\end{equation}
...
\begin{equation} \label{eq:eqgpsC_def}
  \eqgpsChat \equiv \frac{\amacro{p}_s}{\rasub{q}_g} \eval{\pdv{\rasub{q}_g}{\amacro{p}_s}}_{\rate{u} \, = \, \rbsub{u} \, = \, \rasub{u}}
\end{equation}
...
In Eq.~(\ref{eq:C_g_solution}), all terms on the right side except $\amacro{p}_s$ are constant 
for the purposes of the partial derivative. 
...
\begin{equation} 
  \rasub{q}_g = \rorig{q}_g \left\{ 1 + f \left[ \left( z \amacro{p}_s \right)^{m_g} - 1 \right] \right\}^n \; ,
\end{equation}
%
with $f$, $n$, and $z$ being constants defined in the derivation of $\eqspsChat$ above.

Taking the partial derivative of $\rasub{q}_g$ with respect to $\amacro{p}_s$, 
via repeated application of the chain rule, gives

\begin{equation}
  \pdv{\rasub{q}_g}{\amacro{p}_s} = \rorig{q}_g m_g n f z^{m_g} \amacro{p}_s^{m_g-1} 
                \left\{ 1 + \left[ f \left( z \amacro{p}_s \right)^{m_g} - 1 \right] \right\}^{n-1} \; .
\end{equation}
...
\begin{align}
  \eqgpsChat &\equiv \frac{\amacro{p}_s}{\rasub{q}_g} \eval{\pdv{\rasub{q}_g}{\amacro{p}_s}}_{\rate{u} \, = \, \rbsub{u} \, = \, \rasub{u}} \nonumber \\
         &= \frac{\amacro{p}_s}{\cancel{\rorig{q}_g} \left\{ 1 + f \left[ \left( z \amacro{p}_s \right)^{m_g} - 1 \right] \right\}^n } \;
           \cancel{\rorig{q}_g} m_g n f z^{m_g} \amacro{p}_s^{m_g - 1} 
                \left\{ 1 + f \left[ \left( z \amacro{p}_s \right)^{m_g} - 1 \right] \right\} ^{n-1}  \; .
\end{align}
%
Cancelling terms and
combining $\amacro{p}_s$ and $\left\{ 1 + f  \left[ \left( z \amacro{p}_s \right)^{m_g} - 1 \right] \right\}$ 
terms with different exponents gives

\begin{equation} \label{eq:eqgpsC_exact_simple}
  \eqgpsChat = \frac{m_g n f (z \amacro{p}_s)^{m_g}}{1 + f \left[ (z \amacro{p}_s)^{m_g} - 1 \right]} \; .
\end{equation}
%
Back-substituting the constants and simplifying where possible yields

\begin{equation} \label{eq:eqgpsC_exact}
  \eqgpsChat = - \frac{\frac{1}{\rho - 1} \fCs \left( \frac{1 - \fCs}{\fCs} \frac{\amacro{p}_s \rorig{q}_s}{\rorig{C}_g} \right) ^ {\rho / (\rho - 1)} }{1 + \fCs \left[ \left( \frac{1 - \fCs}{\fCs} \frac{\amacro{p}_s \rorig{q}_s}{\rorig{C}_g} \right)^{\rho/(\rho - 1)} - 1 \right]} \; .
\end{equation}
...
\begin{equation}
 \pdv{\eqgpsChat}{\amacro{p}_s} = \frac{m_g^2 n f (z \amacro{p}_s)^{m_g}}{\amacro{p}_s (1 + f \left[ (z \amacro{p}_s)^{m_g} - 1 \right])^2} \; .
\end{equation}

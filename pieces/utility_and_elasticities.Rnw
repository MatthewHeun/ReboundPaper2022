...
\begin{equation} \label{eq:eqopsC_def}
  \eqopsChat \equiv \frac{\amacro{p}_s}{\rasub{q}_g} \eval{\pdv{\rasub{q}_g}{\amacro{p}_s}}_{\rate{u} \, = \, \rbsub{u} \, = \, \rasub{u}}
\end{equation}
...
\begin{equation} \label{eq:eqopsC_exact_simple}
  \eqopsChat = \frac{m_g n f (z \amacro{p}_s)^{m_g}}{1 + f \left[ (z \amacro{p}_s)^{m_g} - 1 \right]} \; .
\end{equation}
%
Back-substituting the constants and simplifying where possible yields

\begin{equation} \label{eq:eqopsC_exact}
  \eqopsChat = - \frac{\frac{1}{\rho - 1} \fCs \left( \frac{1 - \fCs}{\fCs} \frac{\amacro{p}_s \rorig{q}_s}{\rorig{C}_g} \right) ^ {\rho / (\rho - 1)} }{1 + \fCs \left[ \left( \frac{1 - \fCs}{\fCs} \frac{\amacro{p}_s \rorig{q}_s}{\rorig{C}_g} \right)^{\rho/(\rho - 1)} - 1 \right]} \; .
\end{equation}
...
\begin{equation}
 \pdv{\eqopsChat}{\amacro{p}_s} = \frac{m_g^2 n f (z \amacro{p}_s)^{m_g}}{\amacro{p}_s (1 + f \left[ (z \amacro{p}_s)^{m_g} - 1 \right])^2} \; .
\end{equation}

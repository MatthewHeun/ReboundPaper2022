...
%..............................
\paragraph{Compensated own price elasticity of energy service consumption ($\eqspsC$)} 
...
In contrast to the approximate utility model above, 
the compensated own price elasticity of energy service demand ($\eqspsC$)
is not constant in the CES utility model.
Rather, $\eqspsC$ is a function of the post-EEU energy service price ($\amacro{p}_s$).
The definition of $\eqspsC$ is 

\begin{equation} \label{eq:eqspsC_def}
  \eqspsC \equiv \frac{\amacro{p}_s}{\rasub{q}_s} \eval{\pdv{\rasub{q}_s}{\amacro{p}_s}}_{\rate{u} \, = \, \rbsub{u} \, = \, \rasub{u}} \; .
\end{equation}

To find an expression for $\eqspsC$ in the CES utility function, 
we need to first find the partial derivative of an expression for the rate of energy service consumption~($\rate{q}_s$)
as a function of energy service price~($\amacro{p}_s$)
with respect to $\amacro{p}_s$ at constant utility ($\rate{u} = \rbsub{u} = \rasub{u}$)
across the substitution effect.
The equation from which the derivation commences 
was already derived as Eq.~(\ref{eq:q_s_solution}) and is repeated here for convenience.
...
\begin{equation} 
  \rasub{q}_s = \rorig{q}_s \left[ f + g \left( z \amacro{p}_s \right) ^ m \right] ^ n \; .
\end{equation}
...
\begin{equation}
  \pdv{\rasub{q}_s}{\amacro{p}_s} = \rorig{q}_s m n g z^m \amacro{p}_s^{m-1} 
                \left\{ \left[ f + g \left( z \amacro{p}_s \right)^m \right]^{n-1} \right\} \; .
\end{equation}
...
\begin{equation}
  \eqspsC \equiv \frac{\amacro{p}_s}{\rasub{q}_s} \eval{\pdv{\rasub{q}_s}{\amacro{p}_s}}_{\rate{u} \, = \, \rbsub{u} \, = \, \rasub{u}} 
         = \frac{\amacro{p}_s}{\cancel{\rorig{q}_s} \left[ f + g \left( z \amacro{p}_s \right)^m \right]^n } \;
           \cancel{\rorig{q}_s} m n g z^m \amacro{p}_s^{m-1} 
                \left\{ \left[ f + g \left( z \amacro{p}_s \right)^m \right]^{n-1} \right\} \; .
\end{equation}
%
Cancelling terms and
combining $\amacro{p}_s$ and $\left[ f + g \left( z \amacro{p}_s \right)^m \right]$ 
terms with different exponents gives

\begin{equation}
  \eqspsC = \frac{m n g (z \amacro{p}_s)^m}{f + g(z \amacro{p}_s)^m} \; .
\end{equation}
%
Back-substituting the constants and simplifying where possible yields

\begin{equation} \label{eq:eqspsC_exact}
  \eqspsC = - \frac{\frac{1}{1 - \rho} \left( 1 - \fCs \right) \left[ \frac{1 - \fCs}{\fCs} \frac{\amacro{p}_s \rorig{q}_s}{\rorig{C}_o} \right] ^ {\rho / (1 - \rho)} }{\fCs + \left( 1 - \fCs \right) \left[ \frac{1 - \fCs}{\fCs} \frac{\amacro{p}_s \rorig{q}_s}{\rorig{C}_o} \right] ^ {\rho / (1 - \rho)} } \; .
\end{equation}

Eq.~(\ref{eq:eqspsC_exact}) shows that the compensated 
energy service price elasticity of energy service consumption ($\eqspsC$)
under the exact utility model is a function 
of the energy service price after the EEU~($\amacro{p}_s$).


%..............................
\paragraph{Compensated cross price elasticity of other goods consumption ($\eqopsC$)} 
...
under the constraint that utility is unchanged 
($\rate{u} = \rbsub{u} = \rasub{u}$).
To find the compensated cross-price elasticity of other goods consumption ($\eqopsC$),
we follow a similar procedure as for deriving the 
own-price elasticity of energy service consumption ($\eqspsC$), 
...
of other goods consumption($\eqopsC$) being

\begin{equation} \label{eq:eqopsC_def}
  \eqopsC \equiv \frac{\amacro{p}_s}{\rasub{q}_o} \eval{\pdv{\rasub{q}_o}{\amacro{p}_s}}_{\rate{u} \, = \, \rbsub{u} \, = \, \rasub{u}} \; .
\end{equation}

Again, we need to first find the partial
derivative of an expression for the rate of other goods consumption ($\rate{q}_o$) 
as a function of the energy service price ($\amacro{p}_s$) 
with respect to $\amacro{p}_s$ 
at constant utility ($\rate{u} = \rbsub{u} = \rasub{u}$)
across the substitution effect.
The second difference is 
the starting point for this derivation, 
Eq.~(\ref{eq:C_o_solution}) 
(instead of Eq.~(\ref{eq:q_s_solution}) for the derivation of $\eqspsC$ above), 
...
\begin{equation} 
  \rasub{q}_o = \rorig{q}_o \left\{ 1 + f \left[ \left( z \amacro{p}_s \right)^m - 1 \right] \right\}^n \; .
\end{equation}
%
Note that only $m$ is different between the derivations of $\eqspsC$ and $\eqopsC$.

...
under the constraint that utility is unchanged across the substitution effect.
In the CES utility model,
the compensated own price elasticity of energy service demand ($\eqspsC$)
is not constant (as in the approximate utility model).
...
\begin{equation} \label{eq:epsqsC_def}
  \eqspsC \equiv \frac{\amacro{p}_s}{\rasub{q}_s} \frac{\partial \rasub{q}_s}{\partial \amacro{p}_s} \; .
\end{equation}
%

...
across the substitution effect.
This derivation of an expression for $\eqspsC$ in the exact utility model
...
of other goods consumption($\eqopsC$).
...
\end{equation}
%
with $f$, $n$, and $z$ being constants defined in the derivation of $\eqspsC$ above.

...
as the price of the energy service ($\amacro{p}_s$) changes 
as a result of the efficiency increase of the EEU
under the constraint that utility is unchanged across the substitution effect ($\rate{u} = \rbsub{u} = \rasub{u}$).
In the CES utility model,
the compensated own price elasticity of energy service demand ($\eqspsC$)
is not constant (as in the approximate utility model above).
...
across the substitution effect.
That equation was already derived as Eq.~(\ref{eq:q_s_solution}) and repeated here.

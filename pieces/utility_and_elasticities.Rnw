...
prior to the EEU can be computed by
...
and the derived elasticity of subsittution parameter ($\sigma$).
The other elasticities ($\eqspsUCorig$, $\eqspsCorig$, and $\eqopsCorig$)
...
The CES utility is rather different to the CPE model with respect to the 

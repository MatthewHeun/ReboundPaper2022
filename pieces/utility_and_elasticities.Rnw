...
the substitution effect requires a model for device owner behavior,
...
Since in analytical utility models, price elasticities are point-measures,
an operational version of such a model should account for the changing price
elasticity along an indifference curve.\footnote{In principle,
arc elasticities could give an accurate measure of the relationship
between price and quantity change with any EEU by calculating the
percentage price and quantity changes between any two consumption bundles \citep{Allen1934}.
However, the problem is that we are trying to calculate the new consumption bundle
using the elasticity, not the other way around.}
...
under the constraint of constant device owner utility.
...
the basket of other goods ($\rate{q}_o$).


%------------------------------
\subsubsection{Pre-EEU ($\circ$) elasticities} 
...
to indicate it applies across all rebound effects.
...
An expression for the compensated own price elasticity ($\eqspsCorig$)
...
for deriving the compensated cross price elasticity ($\eqopsCorig$):
...
Assuming a given value for the uncompensated own price elasticity ($\eqspsUCorig$), 
...
are helpful for elucidating results derived from the framework,
...
because the consumption bundle has changed (a move along
...
we don't derive $\asub{\sigma}$ here
as the CPE utility model is only an approximation
for vanishingly small changes in energy efficiency.
...
as it should, because all terms are positive, with $\rho$ and $\fCs$ being bounded above by 1.
...
For $\sigma>1$, $n$ is positive, and for $\sigma<1$ negative.
Since we expect $\sigma<1$ (otherwise we have backfire rebound conditions),
the derivative is negative: the compensated own price
elasticity becomes more negative.\footnote{For $\sigma=1$, $m_s=0$ and the derivative
is zero: the Cobb-Douglas special case.} Since the
...
Thus, with $\sigma<1$ and $n$ negative, the compensated cross price
elasticity becomes less positive.

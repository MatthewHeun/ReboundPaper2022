...
values are most likely taken from estimation, if they obtained at all. 
As we show in Part~II, 
...
Elasticities are different for the CPE utility model and the CES utility model.

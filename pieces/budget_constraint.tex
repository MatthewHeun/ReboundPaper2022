...
the product of price ($p_o$) and
the rate ($\rate{q}_o$) of other goods consumption,
$\rorig{C}_{\omd} \equiv \rorig{C}_{\om} + \orig{R}_\omega \rorig{C}_d$,
and after some rearranging gives the budget constraint
used in Eq.~(\ref{eq:budget_constraint}):

\begin{equation}
  \rate{M} - R_\alpha \rate{C}_{cap} - \rate{C}_{\omd} = p_E \frac{\rate{q}_s}{\eta} + p_o \rate{q}_o + \rate{N} \; .
\end{equation}

...
<<ElasticityGraph, fig.cap="Sensitivity of elasticities ($\\varepsilon$) to post-EEU energy service price ($\\amacro{p}_{s}$) for the exact utility model. The solid vertical line indicates the original energy service price ($\\orig{p}_s$). The dashed vertical line indicates the upgraded energy service price ($\\amacro{p}_s$) from the examples.", echo=FALSE, fig.width=6, fig.height=4, fig.align="center">>=
...
ggplot2::ggplot() +
  ...
  ggplot2::geom_vline(data = orig_and_upgrade_ps_df, mapping = ggplot2::aes(xintercept = p_s_star), linewidth = 0.1, linetype = "dashed") +

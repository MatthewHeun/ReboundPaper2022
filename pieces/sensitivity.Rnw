...
eta_sens_params_for_eps <-  list(Car = list(eta_engr_units_star = seq(26, 1000, by = 1)), 
                                 Lamp = list(eta_engr_units_star = seq(7000000, 9000000, by = 5000)))
...
eta_sens_data_for_eps <- ReboundTools::parametric_analysis(original_cases = actual_cases,
  ...
  dplyr::select(Case, p_s_star, e_qs_ps_C_hat, e_qo_ps_C_hat, e_qs_ps_UC_hat, e_qo_ps_UC_hat) %>% 
...
eta_sens_data_for_eps %>% 
  ggplot2::ggplot(mapping = ggplot2::aes(x = p_s_star, y = elasticity, linetype = elasticity_type)) + 
  ggplot2::geom_line() + 
  ggplot2::facet_wrap(facets = "Case", scales = "free_x") +
  MKHthemes::xy_theme()
@
...
<<all_Re_terms_eps_graph, echo=FALSE, fig.cap="Sensitivity of rebound components to uncompensated own price elasticity of energy service demand ($\\eqspsUCorig$). The macro factor is set to its calibrated value ($k = 3$). (Note reversed $x$-axis scale.)", fig.width=6, fig.height=4, fig.align="center">>=
...
ggplot2::ggplot() +
  ...
  ggplot2::scale_color_manual(values = c(e_qs_ps_C_hat = "red", 
                              ...
                              labels = c(e_qs_ps_C_hat = expression(hat(epsilon)[dot(q)[s]*","*p[s]*","*c]), 
                                         ...
                                         e_qo_ps_C_hat = expression(hat(epsilon)[dot(q)[o]*","*p[s]*","*c]), 
                                         e_qo_ps_UC_hat = expression(hat(epsilon)[dot(q)[o]*","*p[s]]))) +

...
\label{sec:elasticities_sensitivity}
%++++++++++++++++++++++++++++++


<<ElasticityGraph, fig.cap="Sensitivity of elasticities ($\\epsilon$) to post-EEU energy service price ($\\amacro{p}_{s}$) for the exact utility model.", echo=FALSE, fig.width=6, fig.height=4, fig.align="center">>=
...
eta_sens_data_for_eps <- ReboundTools::parametric_analysis(original_cases = actual_cases,
  ...
  dplyr::mutate(
    p_s_orig = dplyr::case_when(
      ...
      Case == "Lamp" ~ p_s_orig * 1e06  # Convert to micro$/lm-hr
    ),
    p_s_star = dplyr::case_when(
      Case == "Car"  ~ p_s_star * 100,  # Convert to $/hundred miles
      Case == "Lamp" ~ p_s_star * 1e06  # Convert to micro$/lm-hr
    )
  )
...
# Create a data frame of original and upgrade points.
orig_and_upgrade_df <- eta_sens_data_for_eps %>% 
  dplyr::select(Case, eta_engr_units_orig, p_s_orig) %>% 
  unique() %>% 
  dplyr::mutate(
    # For now, add these manually.  
    # If GS likes the graph, add automatically from another data source.
    eta_engr_units_tilde = c(42, 81800), 
    p_s_tilde = c(6.261905, 1.573350)
  )
...
ggplot2::ggplot() +
  ...
  ggplot2::geom_vline(data = orig_and_upgrade_df, mapping = ggplot2::aes(xintercept = p_s_orig), linewidth = 0.1) +
  # Upgraded energy service price
  ggplot2::geom_vline(data = orig_and_upgrade_df, mapping = ggplot2::aes(xintercept = p_s_tilde), linewidth = 0.1) +
  ...
  ggplot2::scale_color_manual(values = c(e_qs_ps_C_hat = "red", 
                              ...
                              labels = c(e_qs_ps_C_hat = expression(epsilon[q[s]*","*p[s]*","*c]), 
                                         e_qs_ps_UC_hat = expression(epsilon[q[s]*","*p[s]]), 
                                         e_qo_ps_C_hat = expression(epsilon[q[o]*","*p[s]*","*c]), 
                                         e_qo_ps_UC_hat = expression(epsilon[q[o]*","*p[s]]))) +

...
eta_sens_data_for_eps_graph <- eta_sens_data_for_eps %>% 
  dplyr::select(Case, eta_engr_units_star, p_s_star, e_qs_ps_C_hat, e_qo_ps_C_hat, e_qs_ps_UC_hat, e_qg_ps_UC_hat) %>% 
  tidyr::pivot_longer(cols = c(e_qs_ps_C_hat, e_qg_ps_C_hat, e_qs_ps_UC_hat, e_qo_ps_UC_hat), 
...
ggplot2::ggplot() +
  ...
  ggplot2::scale_color_manual(values = c(e_qs_ps_C_hat = "red", 
                              ...
                              labels = c(e_qs_ps_C_hat = expression(hat(epsilon)[dot(q)[s]*","*p[s]*","*c]), 
                                         ...
                                         e_qg_ps_C_hat = expression(hat(epsilon)[dot(q)[o]*","*p[s]*","*c]), 
                                         e_qg_ps_UC_hat = expression(hat(epsilon)[dot(q)[o]*","*p[s]]))) +
  ggplot2::scale_linetype_manual(values = c(e_qs_ps_C_hat = "solid", 
                                            e_qs_ps_UC_hat = "dashed", 
                                            e_qg_ps_C_hat = "solid", 
                                            e_qg_ps_UC_hat = "dashed"), 
                                 labels = c(e_qs_ps_C_hat = expression(hat(epsilon)[dot(q)[s]*","*p[s]*","*c]), 
                                            e_qs_ps_UC_hat = expression(hat(epsilon)[dot(q)[s]*","*p[s]]), 
                                            e_qg_ps_C_hat = expression(hat(epsilon)[dot(q)[o]*","*p[s]*","*c]), 
                                            e_qg_ps_UC_hat = expression(hat(epsilon)[dot(q)[o]*","*p[s]]))) +

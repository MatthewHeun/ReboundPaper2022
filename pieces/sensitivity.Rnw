...
shows the effect of independently varying each of five key rebound parameters
($\amacro{\eta}$, 
  $\amacro{C}_{cap}$, 
  $p_E$, 
  $\eqspsUC$, 
...
$\Sexpr{formatC(unlist(Car_Germany_Re_tot * 100), digits = 2)}$\%
...
with the original uncompensated price elasticity of energy service demand ($\eqspsUC^\circ$).
The effect is exponential, and
total rebound increases with larger negative values of $\eqspsUC^\circ$, as expected.
...
is the larger absolute value of original uncompensated own price elasticity ($\eqspsUC^\circ$) 
...
($\Sexpr{formatC(Re_tot_car_with_lamp_e_ps_qs*100, digits = 2)}$\% for the car and 
$\Sexpr{formatC(Re_tot_lamp_k_3*100, digits = 2)}$\% for the lamp).
...
The sensitivity of post-substition effect price elasticities ($\asub{\varepsilon}$)
...
represents infinite efficiency ($\amacro{\eta}_s \rightarrow \infty$), 
...
a higher price means less consumption, all other things being equal.
...
more variation of price elasticities ($\asub{\varepsilon}$) with energy service price ($\asub{p}_s$)
...
analysts to prefer the CES utility model over the CPE approximation.
...
(See the $Re_{dsub}$ line of Fig.~\ref{fig:all_Re_terms_eta_graph},
which, combined with the near-constant $Re_{isub}$ line,

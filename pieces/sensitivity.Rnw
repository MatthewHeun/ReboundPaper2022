...
<<ElasticityGraph, fig.cap="Sensitivity of elasticities ($\\epsilon$) to post-EEU energy service price ($\\amacro{p}_{s}$)", echo=FALSE, fig.width=6, fig.height=4, fig.align="center">>=
...
eta_sens_data_for_eps_graph <- eta_sens_data_for_eps %>% 
  ...
  dplyr::mutate(
    elasticity_type = factor(elasticity_type, levels = c("e_qo_ps_UC_hat", "e_qo_ps_C_hat", "e_qs_ps_C_hat", "e_qs_ps_UC_hat"))
  )
...
ggplot2::ggplot() +
  ggplot2::geom_vline(data = orig_and_upgrade_df, mapping = ggplot2::aes(xintercept = p_s_orig), linewidth = 0.2) +

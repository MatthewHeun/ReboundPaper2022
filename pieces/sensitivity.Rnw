...
eta_sens_params_for_eps <-  list(Car = list(eta_engr_units_star = seq(26, 1000, by = 1)), 
...
eta_sens_data_for_eps <- ReboundTools::parametric_analysis(original_cases = actual_cases,
  ...
  dplyr::mutate(
    p_s_orig = dplyr::case_when(
      Case == "Car"  ~ p_s_orig * 100,   # Convert to $/hundred miles
...
ggplot2::ggplot() +
  ...
  ggplot2::geom_line() + 
  # ggplot2::labs(x = paste0(expression(p[s]), " [¢/mile (Car) or ", expression(mu), "$/lm-hr (Lamp)]"), 

...
In each subsection below, a figure
shows the effect of independently varying each of six key rebound parameters
($\amacro{\eta}$, 
  $\amacro{C}_{cap}$, 
  $p_E$, 
  $\eqspsUC$, 
...
In many of the graphs, 
nominal values for rebound parameters are shown as points, 
...
<<eta_tilde_takeback_Sdot_sens_graph, echo=FALSE, fig.cap="Expected energy savings rate ($\\Sdot$, solid line) and takeback rate (dashed line) sensitivity to post-EEU efficiency ($\\amacro{\\eta}$). The macro factor is set to its calibrated value ($k = 3$). (Note different $x$- and $y$-axis scales.)", fig.width=6, fig.height=4, fig.align="center">>=
...
Thus, at $\amacro{\eta} = \Sexpr{efficacy_better}$ \lmhr/\Whr, 
...
<<all_Re_terms_eta_graph, echo=FALSE, fig.cap="Sensitivity of rebound components to post-EEU efficiency ($\\amacro{\\eta}$). The macro factor is set to its calibrated value ($k = 3$).", fig.width=6, fig.height=4, fig.align="center">>=
...
<<all_Re_terms_C_cap_graph, echo=FALSE, fig.cap="Sensitivity of rebound components to capital cost ($\\amacro{C}_{cap}$). The macro factor is set to its calibrated value ($k = 3$).", fig.width=6, fig.height=4, fig.align="center">>=
...
<<all_Re_terms_p_E_graph, echo=FALSE, fig.cap="Sensitivity of rebound components to energy price ($p_E$). German energy prices denoted by vertical lines. The macro factor is set to its calibrated value ($k = 3$).", fig.width=6, fig.height=4, fig.align="center">>=
...
%++++++++++++++++++++++++++++++
\subsection{Effect of original uncompensated own prices elasticity ($\eqspsUC^\circ$)} 
...
<<all_Re_terms_eps_graph, echo=FALSE, fig.cap="Sensitivity of rebound components to uncompensated energy service price elasticity of energy demand ($\\eqspsUC$). The macro factor is set to its calibrated value ($k = 3$). (Note reversed $x$-axis scale.)", fig.width=6, fig.height=4, fig.align="center">>=
...
%regardless of $\amacro{p}_s$,
%such that as $\amacro{p}_s$ declines with increased device efficiency ($\amacro{\eta}_s$),
%consumption of the energy service rises. 
%The compensated cross price elasticity ($\eqopsChat$) is positive,
%regardless of $\amacro{p}_s$
%such that as $\amacro{p}_s$ declines with increased $\amacro{\eta}_s$,
%consumption of other goods falls.
The compensated own price elasticity ($\eqspsChat$) is negative
and hte compensate cross price elasticity ($\eqopsChat$) is positive.
...
eta_sens_data_for_eps_graph <- eta_sens_data_for_eps %>% 
  dplyr::select(Case, eta_engr_units_star, p_s_star, e_qs_ps_C_hat, e_qo_ps_C_hat, e_qs_ps_UC_hat, e_qo_ps_UC_hat) %>% 
  tidyr::pivot_longer(cols = c(e_qs_ps_C_hat, e_qo_ps_C_hat, e_qs_ps_UC_hat, e_qo_ps_UC_hat), 
                      names_to = "elasticity_type", 
                      values_to = "elasticity") %>% 
  dplyr::mutate(
    elasticity_type = factor(elasticity_type, levels = c("e_qo_ps_C_hat",
                                                         "e_qo_ps_UC_hat",
                                                         "e_qs_ps_C_hat",
                                                         "e_qs_ps_UC_hat"))
  )
...
ggplot2::ggplot() +
  ...
  ggplot2::scale_color_manual(values = c(e_qs_ps_C_hat = "red", 
                                         e_qs_ps_UC_hat = "red", 
                                         e_qo_ps_C_hat = "black", 
                                         e_qo_ps_UC_hat = "black"), 
                              labels = c(e_qs_ps_C_hat = expression(hat(epsilon)[q[s]*","*p[s]*","*c]), 
                                         e_qs_ps_UC_hat = expression(hat(epsilon)[q[s]*","*p[s]]), 
                                         e_qo_ps_C_hat = expression(hat(epsilon)[q[o]*","*p[s]*","*c]), 
                                         e_qo_ps_UC_hat = expression(hat(epsilon)[q[o]*","*p[s]]))) +
  ggplot2::scale_linetype_manual(values = c(e_qs_ps_C_hat = "solid", 
                                            e_qs_ps_UC_hat = "dashed", 
                                            e_qo_ps_C_hat = "solid", 
                                            e_qo_ps_UC_hat = "dashed"), 
                                 labels = c(e_qs_ps_C_hat = expression(hat(epsilon)[q[s]*","*p[s]*","*c]), 
                                            e_qs_ps_UC_hat = expression(hat(epsilon)[q[s]*","*p[s]]), 
                                            e_qo_ps_C_hat = expression(hat(epsilon)[q[o]*","*p[s]*","*c]), 
                                            e_qo_ps_UC_hat = expression(hat(epsilon)[q[o]*","*p[s]]))) +
...
compared to the car example.
Using the constant price elasticity (CPE) utility model may have acceptably
small errors in the lamp example. 
...
analysts implementing an analytical rebound calculation
...
%Thus, substitution rebound ($Re_{sub}$) is attenuated as efficiency increases.

...
because we assume an upgraded device with equal performance 
as the original device and, in the absence of behavior changes,
...
developing both CPE and CES models of determining
...
%expected to be negative ($\eqspsC < 0$), % GS: this is too repetitive for my taste, it was already said two times in teh previous 2 paragraphs.
such that, e.g.\
...
for infinitesimally small energy service price changes
($\Delta \aempl{p}_s \equiv \aempl{p}_s - \bempl{p}_s \approx 0$)
when an analytical utility model is used. They also 
provide reasonable approximations for a one or two per cent change.
